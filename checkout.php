...
if (isset($_SESSION['userID'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        // Check if selected_plan is set
        $selected_plan = isset($_POST['selected_plan']) ? $_POST['selected_plan'] : '';
        ...
        $conn->autocommit(false); // Disable autocommit if needed
        $stmt = $conn->prepare("UPDATE membership SET membership_exp = ?, membership_status = ?, total_price = ? WHERE userID = ?");
        $stmt->bind_param("sssi", $expiryDate, $selected_plan, $price, $userID);
        ...
        if ($stmt->execute()) {
            ...
            echo "Statement preparation failed: (" . $conn->errno . ") " . $conn->error; // Show preparation error
        }
    }
